...
router.post('/', upload.single('image'), async (req, res, next) => {
  ...
  try {
    ...
    const response = {
      ...
      // 4. CROSS-VERIFICATION ANALYSIS (NEW)
      crossVerification: {
        matchPercentage: intelligentAnalysis.crossVerification.matchPercentage,
        confidence: intelligentAnalysis.crossVerification.confidence,
        overallStatus: intelligentAnalysis.crossVerification.overallStatus,
        agreements: intelligentAnalysis.crossVerification.agreements,
        conflicts: intelligentAnalysis.crossVerification.conflicts,
        sensorCorrelation: intelligentAnalysis.crossVerification.sensorCorrelation
      },
      
      // 5. DASHBOARD SUMMARY (NEW)
      dashboardSummary: {
        healthScore: intelligentAnalysis.healthScore,
        emergencyLevel: intelligentAnalysis.dashboardSummary.emergencyLevel,
        status: intelligentAnalysis.dashboardSummary.status,
        trend: intelligentAnalysis.dashboardSummary.trend,
        lastUpdated: intelligentAnalysis.dashboardSummary.lastUpdated,
        insights: intelligentAnalysis.dashboardSummary.insights,
        reminders: intelligentAnalysis.dashboardSummary.reminders,
        sensorStatus: intelligentAnalysis.dashboardSummary.sensorStatus
      },
      
      // 6. SPECIFIC DEFICIENCIES DETECTED
      ...
      finalDiagnosis: {
        ...
        confidence: aiResult.confidence,
        message: intelligentAnalysis.intelligentDiagnosis.message,
        healthScore: intelligentAnalysis.healthScore // Added health score
      }
    };

    console.log('✅ === INTELLIGENT ANALYSIS COMPLETE ===');
    console.log(`📋 Verdict: ${intelligentAnalysis.intelligentDiagnosis.verdict}`);
    console.log(`📊 Health Score: ${intelligentAnalysis.healthScore}%`);
    console.log(`🤝 Cross-Verification: ${intelligentAnalysis.crossVerification.matchPercentage}% match`);
